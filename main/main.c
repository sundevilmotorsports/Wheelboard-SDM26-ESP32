...
void MLX90642_Initialize() {
  ESP_LOGW(TAG,
           "MLX90642 library not present; temperature sensor task disabled");
}
#endif
...
void app_main(void) {
  ...
  // Fast blink sequence to signify successful boot
  ESP_LOGI(TAG, "Boot successful! Running startup blink sequence...");
  for (int i = 0; i < 10; i++) {
    led_util_set_color(0, 0, 255); // Blue
    vTaskDelay(pdMS_TO_TICKS(100));
    led_util_clear();
    vTaskDelay(pdMS_TO_TICKS(100));
  }
  ESP_LOGI(TAG, "Startup sequence complete. System is running.");
